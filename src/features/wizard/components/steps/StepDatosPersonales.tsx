...
export function StepDatosPersonales() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (gender && age !== null && age >= 10 && age <= 99 && hasChildren !== null) {
      if (gender === 'Femenino' && isPregnant === null) return;
      nextStep();
    }
  };

  const isFormValid = gender !== null && age !== null && age >= 10 && age <= 99 && hasChildren !== null && (gender !== 'Femenino' || isPregnant !== null);
    ...
    <form onSubmit={handleSubmit} className="flex flex-col flex-grow h-full content-start">
      ...
      <fieldset className="flex-grow mb-xl border-0 p-0 m-0 flex flex-col gap-lg">
        <div className="flex flex-col gap-xs">
          <label className="font-label-md text-label-md text-on-surface" htmlFor="gender-select">{t("gender")}</label>
          <CustomSelect
            id="gender-select"
            options={genderOptions}
            value={gender}
            onChange={(val) => setGender(val as 'Femenino' | 'Masculino' | 'Otro')}
        ...
        <div className="flex flex-col gap-xs">
          ...
          <div className="flex gap-4 mt-2">
            <label className="relative flex items-center cursor-pointer gap-2">
              ...
              />
              <span className="font-body-md text-body-md text-on-surface">{t("yes")}</span>
            </label>
            <label className="relative flex items-center cursor-pointer gap-2 ml-4">
              <input 
                ...
                className="w-5 h-5 text-secondary border-outline-variant focus:ring-secondary focus:ring-offset-2"
              />
              <span className="font-body-md text-body-md text-on-surface">{t("no")}</span>
            </label>
          </div>
        </div>
        ...
        {gender === 'Femenino' && (
          <div className="flex flex-col gap-xs animate-in fade-in slide-in-from-top-4 duration-300 bg-surface-container p-4 rounded-lg mt-2">
            ...
            <p className="font-label-sm text-label-sm text-on-surface-variant mt-2">{t("maternitySupport")}</p>
          </div>
        )}
      </fieldset>
